...
import { createMatrix, createNextMatrix, deepSearch, widthSearch, findNextNodes, findNodeDegree, LoopDejkstra, LoopBellmanFord } from '../components/Math';
...
const Task4 = () => {
    ...
    // - Дейкстра
    const [Dejkstra, setDejkstra] = useState([])

    // - Беллмана Форда
    const [BellmanFord, setBellmanFord] = useState([])
    ...
    function handleSubmit() {
        if( V > 0 && R >= V-1 && R <= (V * (V - 1) / 2)) {
            ...
            let dejk = []
            dejk = LoopDejkstra(matrix._data, V)
            setDejkstra(dejk)

            let bford = []
            bford = LoopBellmanFord(matrix._data, V)
            setBellmanFord(bford)
        }
    }
            ...
            <div class="sidebar">
                ...
                <div class='wrapper'>
                    <div class='match-result'>
                        <p>Генерируется случайный связный неориентированный граф без петель и кратных рёбер в виде матрицы смежности. </p>
                        <p>Реализовать алгоритмы поиска кратчайших путей: </p>
                        <p>алгоритм Дейкстры, алгоритм Беллмана-Форда, Алгоритм Флойда-Уоршелла.</p>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            { item.widget === 'first' && 
                                <div style={{ textAlign: 'center', marginTop: '15px'}}>
                                    <a>Алгорит Дейкстра</a>
                                    <div class='scheme'> 
                                        { N.map((item, index) => (
                                            <div class='rowN' key={index}>
                                                <div class='el'>{item}</div>
                                                    <div class='rowP'>
                                                        <div class='el'>{ Dejkstra[item-1] }</div>
                                                    </div>
                                            </div>                                        
                                        )) }
                                    </div>
                            ...
                            { item.widget === 'second' &&
                                <div style={{ textAlign: 'center', marginTop: '15px'}}>
                                <a>Алгорит Беллмана-Форда</a>
                                <div class='scheme'> 
                                    { N.map((item, index) => (
                                        <div class='rowN' key={index}>
                                            <div class='el'>{item}</div>
                                                <div class='rowP'>
                                                    <div class='el'>{ BellmanFord[item-1] }</div>
                                                </div>
                                        </div>                                        
                                    )) }
                                </div>
